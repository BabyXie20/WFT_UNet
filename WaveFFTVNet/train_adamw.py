...
from network_models.network_backbone import Waveformer
from networks.WaveCoFormer import Wavecoformer
...
def parse_args():
    ...
    parser.add_argument("--val_start_iter", type=int, default=9200, help="start val eval at this iter (inclusive)")
    ...
    parser.add_argument("--dropout_path_rate", type=float, default=0.0)
    parser.add_argument("--use_checkpoint", action="store_true", help="use gradient checkpointing in SwinUNETR")
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=["0009"],
        help="these case ids are always assigned to the train split before random splitting",
    )
    parser.add_argument(
        "--snapshot_extra",
        type=str,
    ...
    parser.add_argument("--multi_run_count", type=int, default=3, help="number of sequential runs in multi-run mode")
...
def build_custom_splits_from_json(
    ...
    force_train_cases: Optional[List[str]] = None,
    ...
    excluded_set = set()
    if exclude_cases:
        excluded_set = {_normalize_case_id(x) for x in exclude_cases if str(x).strip()}

    forced_train_set = set()
    if force_train_cases:
        forced_train_set = {_normalize_case_id(x) for x in force_train_cases if str(x).strip()}

    overlap = excluded_set & forced_train_set
    if overlap:
        print(f"[SPLIT][WARN] force_train_cases override exclude_cases for: {sorted(list(overlap))}")
        excluded_set -= overlap

    if excluded_set:
        before = len(pool)
        pool = [d for d in pool if not _match_case_set(d, excluded_set)]
        after = len(pool)
        print(f"[SPLIT] excluded cases={sorted(list(excluded_set))} | removed={before-after} | remain={after}")

    forced_train_files: List[Dict[str, Any]] = []
    if forced_train_set:
        forced_train_files = [d for d in pool if _match_case_set(d, forced_train_set)]
        pool = [d for d in pool if not _match_case_set(d, forced_train_set)]

        found_force_ids = set()
        for d in forced_train_files:
            for k in ("image", "label"):
                base = os.path.basename(str(d.get(k, "") or ""))
                found_force_ids.update(_extract_case_ids_from_basename(base))

        missing = sorted(list(forced_train_set - found_force_ids))
        if missing:
            print(f"[SPLIT][WARN] force_train_cases not found in pool: {missing}")

        print(
            f"[SPLIT] forced train cases={sorted(list(forced_train_set))} | "
            f"found={len(forced_train_files)} | remaining_pool={len(pool)}"
        )
    ...
    if val_num + test_num > total_rest:
        overflow = val_num + test_num - total_rest
        test_num = max(0, test_num - overflow)

    remain_rest = total_rest - (val_num + test_num)

    if train_num < 0:
        train_extra_num = remain_rest
    else:
        if train_num < len(forced_train_files):
            print(
                f"[SPLIT][WARN] train_num={train_num} < forced_train_found={len(forced_train_files)}; "
                f"train split will still keep all forced cases."
            )
        requested_extra = max(0, train_num - len(forced_train_files))
        train_extra_num = min(requested_extra, remain_rest)

    val_files = pool[:val_num]
    test_files = pool[val_num: val_num + test_num]
    train_files = forced_train_files + pool[val_num + test_num: val_num + test_num + train_extra_num]
...
def _case_diagonal_mm(spatial_shape: Tuple[int, int, int], spacing: Tuple[float, float, float]) -> float:
    return float(math.sqrt(sum((float(s) * float(sp)) ** 2 for s, sp in zip(spatial_shape, spacing))))


def _sanitize_assd_case(
    assd_case: torch.Tensor,
    pred_case: torch.Tensor,
    true_case: torch.Tensor,
    pixdim: Tuple[float, float, float],
) -> torch.Tensor:
    assd_case = assd_case.detach().cpu().float().clone()
    diag_mm = _case_diagonal_mm(tuple(int(x) for x in pred_case.shape[1:]), pixdim)

    fg_classes = assd_case.numel()
    for idx in range(fg_classes):
        c = idx + 1
        pred_nonempty = bool(torch.count_nonzero(pred_case[c]).item() > 0)
        true_nonempty = bool(torch.count_nonzero(true_case[c]).item() > 0)

        if (not pred_nonempty) and (not true_nonempty):
            assd_case[idx] = float("nan")
        elif pred_nonempty ^ true_nonempty:
            assd_case[idx] = diag_mm
        elif not torch.isfinite(assd_case[idx]):
            assd_case[idx] = diag_mm
...
def run_test_best_and_export_excel(
    ...
    for batch in pbar:
        ...
        dice_valid = torch.isfinite(dice_c_cpu)
        sum_dice += torch.where(dice_valid, dice_c_cpu, torch.zeros_like(dice_c_cpu))
        ...
        if fg_classes > 0:
            ...
            hd95_valid = torch.isfinite(hd95_cpu)
            sum_hd95 += torch.where(hd95_valid, hd95_cpu, torch.zeros_like(hd95_cpu))
            cnt_hd95 += hd95_valid.double()

            assd_cpu = assd_case.double()
            assd_valid = torch.isfinite(assd_cpu)
            sum_assd += torch.where(assd_valid, assd_cpu, torch.zeros_like(assd_cpu))
            cnt_assd += assd_valid.double()

            nsd_cpu = nsd_case.double()
            nsd_valid = torch.isfinite(nsd_cpu)
            sum_nsd += torch.where(nsd_valid, nsd_cpu, torch.zeros_like(nsd_cpu))
    ...
    print("\n================ TEST (best.pt) ================")
    print(f"TEST dice_mean_fg         : {dice_mean_fg:.6f}")
    print(f"TEST hd95_mean_excl_bg    : {hd95_mean_excl_bg:.6f}")
    print(f"TEST assd_mean_excl_bg    : {assd_mean_excl_bg:.6f}")
    print(f"TEST nsd_mean_excl_bg     : {nsd_mean_excl_bg:.6f} (tol={nsd_tol_mm} mm)")
    print("================================================\n")
    ...
    xlsx_path = os.path.join(output_dir, "test_metrics.xlsx")
    summary = {
    ...
    print(f"[TEST] Excel report saved: {xlsx_path}")
...
def run_single_experiment(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if args.val_separate:
        train_files, val_files, test_files = build_custom_splits_from_json(
            ...
            force_train_cases=args.force_train_cases,
        )
    ...
    print(f"[FORC] force_train_cases={args.force_train_cases if args.force_train_cases else '[]'}")
    ...
    cfg.update(
        {
            ...
            "model_name": "monai.networks.nets.SwinUNETR",
            ...
            "force_train_cases": list(args.force_train_cases),
    ...
    writer.add_text("meta/model_name", "monai.networks.nets.SwinUNETR", 0)
    writer.add_text("meta/optimizer_name", "AdamW", 0)
    writer.add_text("meta/feature_size", str(args.feature_size), 0)
    writer.add_text("meta/use_checkpoint", str(bool(args.use_checkpoint)), 0)
    writer.add_text("meta/exclude_cases", ",".join(list(args.exclude_cases)) if args.exclude_cases else "", 0)
    writer.add_text("meta/force_train_cases", ",".join(list(args.force_train_cases)) if args.force_train_cases else "", 0)
    ...
    #model = UNETR_PP(in_channels=1, out_channels=14, img_size=(96, 96, 96)).to(device)
    #model = Waveformer(img_size=(96, 96, 192),patch_size=2,in_chans=1, out_chans=14, depths=[2, 2, 2, 2],feat_size=[48, 96, 192, 384],num_heads=[3, 6, 12, 24],drop_path_rate=0.1,use_checkpoint=False).to(device)
    model = Wavecoformer(
        img_size=(96, 96, 96),
        in_channels=1,        
        out_channels=14,       
    ...
    if os.path.isfile(best_path):
        ...
        test_res = run_test_best_and_export_excel(
            model=model,
        ...
        test_metrics = {
            ...
            "test_dice_mean_fg": safe_float(float(test_res["dice_mean_fg"])),
            "test_hd95_mean_excl_bg": safe_float(float(test_res["hd95_mean_excl_bg"])),
            "test_assd_mean_excl_bg": safe_float(float(test_res["assd_mean_excl_bg"])),
            "test_nsd_mean_excl_bg": safe_float(float(test_res["nsd_mean_excl_bg"])),
            "nsd_tol_mm": safe_float(float(test_res["nsd_tol_mm"])),
            ...
            "test_assd_per_class_excl_bg": tensor_to_float_list(test_res["assd_per_class_excl_bg"]),
            "test_nsd_per_class_excl_bg": tensor_to_float_list(test_res["nsd_per_class_excl_bg"]),
            "test_case_rows_sorted_by_fg_dice": test_res["case_rows"],
            "excel_path": test_res["excel_path"],
            "class_names": class_names,
            "class_labels": CLASS_LABELS,
        ...
        print(f"[WARN] best.pt not found, skip testing. expected: {best_path}")
...
def save_multi_run_summary_excel(xlsx_path: str, run_results: List[Dict[str, Any]], class_names: List[str]) -> None:
    wb = Workbook()

    ws = wb.active
    ws.title = "run_metrics"
    run_headers = [
        "run_index", "run_name", "split_seed", "seed",
        "dice_mean_fg", "hd95_mean_excl_bg", "assd_mean_excl_bg", "nsd_mean_excl_bg",
        "test_metrics_json", "test_metrics_xlsx",
    ]
    ...
    for r in run_results:
        ws.append([
            ...
            r.get("test_metrics_json"),
            r.get("test_metrics_xlsx"),
        ])
    ws.freeze_panes = "A2"
    for c in ws[1]:
    ...
    ws2.append(["metric", "mean_over_5_runs"])
    ws2.append(["dice_mean_fg", _nanmean([r.get("dice_mean_fg") for r in run_results])])
    ws2.append(["hd95_mean_excl_bg", _nanmean([r.get("hd95_mean_excl_bg") for r in run_results])])
    ws2.append(["assd_mean_excl_bg", _nanmean([r.get("assd_mean_excl_bg") for r in run_results])])
    ws2.append(["nsd_mean_excl_bg", _nanmean([r.get("nsd_mean_excl_bg") for r in run_results])])
    ws2.freeze_panes = "A2"
    for c in ws2[1]:
        c.font = Font(bold=True)
        c.alignment = Alignment(horizontal="center")
    ...
    for c in range(1, len(class_names)):
        ...
        ws3.append([c, class_names[c], _nanmean(dice_vals), _nanmean(hd95_vals), _nanmean(assd_vals), _nanmean(nsd_vals)])
...
def main():
    ...
    for i in range(int(args.multi_run_count)):
        s = int(args.multi_run_seed_start) + i * int(args.multi_run_seed_step)
        seed_pairs.append((s, s))
    ...
    for i, (split_seed, seed) in enumerate(seed_pairs, start=1):
        sub_run_name = f"{base_run_name}_run{i}_split{split_seed}_seed{seed}"
        cmd = [
            sys.executable,
        ...
        print(f"\n[MULTI-RUN] ({i}/{len(seed_pairs)}) split_seed={split_seed}, seed={seed}")
        print("[MULTI-RUN] cmd:", " ".join(cmd))
        subprocess.run(cmd, check=True)

        sub_out_dir = os.path.join(args.output_root, f"btcv_run_{sub_run_name}")
        metrics_json = os.path.join(sub_out_dir, "test_metrics.json")
        metrics_xlsx = os.path.join(sub_out_dir, "test_metrics.xlsx")
        ...
        all_results.append({
            ...
            "seed": seed,
            "dice_mean_fg": safe_float(tm.get("test_dice_mean_fg")),
            "hd95_mean_excl_bg": safe_float(tm.get("test_hd95_mean_excl_bg")),
            "assd_mean_excl_bg": safe_float(tm.get("test_assd_mean_excl_bg")),
            "nsd_mean_excl_bg": safe_float(tm.get("test_nsd_mean_excl_bg")),
            "dice_per_class_incl_bg": tm.get("test_dice_per_class_incl_bg", []),
            "hd95_per_class_excl_bg": tm.get("test_hd95_per_class_excl_bg", []),
            "assd_per_class_excl_bg": tm.get("test_assd_per_class_excl_bg", []),
            "nsd_per_class_excl_bg": tm.get("test_nsd_per_class_excl_bg", []),
            "test_metrics_json": metrics_json,
            "test_metrics_xlsx": metrics_xlsx,
        })

    summary_xlsx = os.path.join(args.output_root, f"btcv_run_{base_run_name}_5runs_summary.xlsx")
    save_multi_run_summary_excel(summary_xlsx, all_results, class_names)
    print(f"[MULTI-RUN] summary excel saved: {summary_xlsx}")

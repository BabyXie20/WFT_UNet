...
class VSNet(nn.Module):
    def __init__(
        ...
        self.gate2 = Gate(
            in_channels_up=feature_size,
            in_channels_down=2 * feature_size,
            out_channels=feature_size
        )
        self.gate3 = Gate(
            in_channels_up=2 * feature_size,
            in_channels_down=4 * feature_size,
            out_channels=2 * feature_size
        )
        self.gate4 = Gate(
            in_channels_up=4 * feature_size,
            in_channels_down=8 * feature_size,
            out_channels=4 * feature_size
        )
        ...
        # 可选：深监督输出，也改成14类 logits
        self.deep_supervision2 = SegHead(
            in_channels=feature_size * 2,
            out_channels=out_channels
        )

        self.deep_supervision3 = SegHead(
    ...
    def forward(self, x, deep_supervision: bool = False):
        ...
        if deep_supervision:
            ds2 = self.deep_supervision2(up2)   # [B, 14, D/2, H/2, W/2]
            ds3 = self.deep_supervision3(up3)   # [B, 14, D/4, H/4, W/4]

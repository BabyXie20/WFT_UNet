...
# python train.py --topk 1
rot = np.deg2rad(30.0)

DEFAULT_NSD_TOL_MM = 1.0

#python train_btcv.py --single_run
def parse_args():
...
def save_test_metrics_excel(
    ...
    # Long format per-case per-class
...
def run_test_best_and_export_excel(
    ...
    def _to_case_chwd(x: torch.Tensor, n_cls: int) -> torch.Tensor:
        ...
        Accept common shapes produced by MONAI post transforms:
          - [C, H, W, D]
          - [1, C, H, W, D]
          - [C, 1, H, W, D]  (rare, defensive)
        """
    ...
    # Per-class aggregation (mean over cases, ignore NaNs)
    ...
    for batch in pbar:
        ...
        # 当前 test loader 的 batch_size=1，这里显式按单 case 处理
        ...
        # -------------------------
        # HD95 / ASSD / NSD per class (foreground only)
        # IMPORTANT:
        #   list-mode input must be CHWD, i.e. [pred_case], [true_case]
        #   spacing length must match spatial dim (=3 here)
        # -------------------------
        if fg_classes > 0:
            # HD95
            if compute_hd95:
        ...
        # Long-format per case per class rows for Excel
        for c in range(num_classes):
            row = {
                "case_id": case_id,
                "class_index": c,
...
def main():
    ...
    #model = VSNet(in_channels=1, out_channels=14).to(device)
    #model = UNETR(in_channels=1, out_channels=14, img_size=(96, 96, 96)).to(device)
    ...
    # =========================
    # TEST: only best.pt (no top-k ensemble)
    # =========================
